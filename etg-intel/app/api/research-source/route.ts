...
import { fetchRSS, fetchCanadianArchitect } from '@/lib/research'

// RSS and page fetches only — fast enough for 10s Hobby limit
// Web search runs client-side directly against Anthropic API (no timeout)
export const maxDuration = 10

export async function POST(request: NextRequest) {
  const { source } = await request.json() as {
    source: 'dcn' | 'onsite' | 'canarchitect'
  }
